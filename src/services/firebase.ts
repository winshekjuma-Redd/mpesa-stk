...
const getAccessToken = async () => {
  ...
  const signature = crypto.sign('RSA-SHA256', Buffer.from(unsignedJwt), env.FIREBASE_PRIVATE_KEY);
...
const decodeFields = (fields: Record<string, any>) =>
  Object.fromEntries(Object.entries(fields).map(([key, value]) => [key, decodeValue(value)]));

const encodeFields = (data: Record<string, any>) =>
...
const firebaseRequest = async (path: string, init: RequestInit = {}) => {
...
const runQuery = async (field: string, value: string, extraFilters: Record<string, string> = {}) => {
  const filters = [
  ...
  const data = await firebaseRequest(':runQuery', {
    ...
    body: JSON.stringify({
      structuredQuery: {
        from: [{ collectionId: env.FIRESTORE_TRANSACTIONS_COLLECTION }],
  ...
  return decodeDocument(data.find((entry: any) => entry.document)?.document);
};

export const findTransactionByReference = async (reference: string) =>
  await runQuery('reference', reference) || await runQuery('internalReference', reference);

export const insertTransaction = async (transaction: Record<string, any>) => {
  await firebaseRequest(`/${env.FIRESTORE_TRANSACTIONS_COLLECTION}?documentId=${encodeURIComponent(transaction.id)}`, {
    method: 'POST',
...
export const updateTransactionByReference = async (matchValue: string, updates: Record<string, any>) => {
  const transaction = await findTransactionByReference(matchValue);
  if (!transaction?.id) return null;

  const updateMask = Object.keys(updates).map((key) => `updateMask.fieldPaths=${encodeURIComponent(key)}`).join('&');
  const data = await firebaseRequest(`/${env.FIRESTORE_TRANSACTIONS_COLLECTION}/${encodeURIComponent(transaction.id)}?${updateMask}`, {
    method: 'PATCH',
